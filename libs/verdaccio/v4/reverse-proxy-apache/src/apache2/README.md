...
```bash
npx nx run verdaccio-v4-reverse-proxy-nginx-apache:up --mode=apache2
```

To recreate the nginx image you can force the build.

```bash
npx nx run verdaccio-v4-reverse-proxy-nginx-apache:recreate --mode=apache2
...
```bash
nx run verdaccio-v4-reverse-proxy-nginx-apache:force-recreate --mode=apache2
```
