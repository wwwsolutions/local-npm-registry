...
```bash
npx nx run verdaccio-v6-reverse-proxy-apache:up --mode=apache2
```

To recreate the nginx image you can force the build.

```bash
npx nx run verdaccio-v6-reverse-proxy-apache:recreate --mode=apache2
...
```bash
nx run verdaccio-v6-reverse-proxy-apache:force-recreate --mode=apache2
```
